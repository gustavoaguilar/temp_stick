...
int init_serial(std::string path, int& serial_port, struct termios& tty){
    ...
    if (serial_port < 0){
        std::cout << "Failed to open device: "<< path <<"\n";
        return -1;
    }
...
int main(int argc, char const *argv[]){
    ...
    #else
        if(init_serial("../Cpp/simulator/virtual-device", serial_port, tty)){
    #endif
            exit(1);
        }
    ...
    while(true){
        ...
        if(!dev.parseSerialLine(data)){
            std::cout << "Failed parsing data\n";
        }
        std::cout << "Parsed data: " << dev.to_string() << std::endl;
